...
- 三列工作区：左侧节点设置，中间提示词、参数与请求记录（逐条显示每次尝试的节点、协议、请求 URL 和成败原因，便于排查失败节点），右侧结果预览。
- 参考图上传：支持文件选择、拖拽到虚线区域、以及直接 Ctrl+V 粘贴截图，最多 8 张，支持逐张移除。本地转成的 `data:image` 用于缩略图预览；OpenAI 兼容与异步接口以 multipart 直接上传原文件，Chat 和自定义接口随请求发送内联数据。
- 草稿自动保存：提示词、尺寸、质量和数量随输入即时存入本机，刷新或下次打开自动恢复；参考图不在草稿内，刷新后需要重新添加。
- 快捷键：Ctrl+Enter（或 Cmd+Enter）直接触发生成。
- 深色模式：自动跟随系统偏好，也可以手动切换；页面加载时用内联脚本提前应用主题，避免深色用户看到白屏闪烁。
- 结果自动清理：远程链接按接口返回的过期时间清理，接口没给过期时间时默认保留 1 小时；内联 `data:image` 结果不过期，只受存储配额和手动清理影响。点「清理链接」会一次性清空全部本地结果记录。
...
- 本地数据共四个 localStorage 键：`simple-img-static.nodes.v1`（节点配置，含明文 API Key）、`simple-img-static.draft.v1`（提示词与参数草稿）、`simple-img-static.links.v1`（结果记录）、`simple-img-static.theme.v1`（主题偏好）。页面上的「清理链接」只清空结果记录；要彻底清除（包括 API Key），请逐个删除节点，或直接在浏览器里清除本站点数据。
...
- 异步响应里的 `poll_url` 只有与节点完全同源（协议+主机+端口）时才会使用，跨源的绝对地址会被忽略并回退到默认轮询路径，避免 `Authorization` 头被发往其他地方。

## 使用方式

直接用浏览器打开 `index.html` 即可，不需要安装依赖或启动服务。

1. 在「API 节点」面板里填写节点名称、base_url、API Key、模型和接入协议。
2. 保存并启用至少一个节点。
3. 在中间列填写提示词、尺寸（1024x1024、4:3、3:4、16:9、9:16 或 auto）、质量（默认、auto、low、medium、high）和生成数量（1-4 张）。尺寸选 auto、质量选默认时，请求中不携带对应字段，交给接口按默认值处理。
4. 可选上传参考图：点击选择、拖进虚线框，或直接粘贴剪贴板里的截图。
5. 点击生成（或按 Ctrl+Enter），右侧「结果显示」会出现预览图、图片链接和复制、原图、下载按钮；点击预览图可在新标签页查看原图。

生成进行中，节点的增删改和参考图的增删会被锁定，只能点「停止」中断；提示词等参数仍可编辑，供下一次生成使用。

## 节点说明

Base URL 根据协议有不同含义：

- `OpenAI` / `自动`：填写接口根地址，例如 `https://api.example.com` 或 `https://api.example.com/v1`。无参考图时提交到 `/v1/images/generations`；带参考图时自动改为 `/v1/images/edits`，参考图以 multipart 形式上传，字段名为 `image[]`。
...
`自动` 协议按顺序尝试候选：普通节点为 OpenAI 兼容 → 异步 → Chat；已知异步中转节点（fnuu.net）为 异步 → OpenAI 兼容 → Chat。自动模式不会尝试 `自定义` 协议，需要直连 Base URL 时请显式选自定义。

重试次数按协议候选分别计算：某个协议失败会先重试该协议 N 次，再切换下一个候选，因此自动协议下单个节点最多发起 3×(N+1) 次请求。所有启用节点按排序依次尝试，任一节点成功即停止。
...
- 轮询容错：网络错误、单次超时、5xx/429 或非 JSON 响应最多容忍连续 2 次，第 3 次连续失败会放弃任务（仍受 10 分钟总预算约束）；4xx（除 429）说明任务查询本身被拒绝，会立即失败并显示接口返回的错误。
- 轮询地址：优先使用与节点完全同源（协议+主机+端口）的 `poll_url`；跨源的绝对 `poll_url` 会被忽略，回退到 `/async/images/{task_id}`。
